...
CData.rho = [];
[CData, dd] = Read_Data_MT('Samtex_ObservedData_Max_out.dat', CData);

out_dir = fullfile('data', 'observed_data');
if ~exist(out_dir, 'dir'); mkdir(out_dir); end

save(fullfile(out_dir, 'SAMTEX_Max_Data.mat'),'CData',"dd");
...
for j = 1:length(CData.MT.Stat.lon)
    ...
    for i = 1:length(site_names)
       if strcmpi(strtrim(CData.MT.Stat.code(j,:)), site_names{i})
